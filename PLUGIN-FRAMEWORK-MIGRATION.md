...
#### Step 7: Copy Import Blocks to Configuration

Copy the generated import blocks from `import.tf` to your main configuration file:

```bash
# Append import blocks to your main.tf
cat import.tf >> main.tf
```

Your `main.tf` should now contain both your non instana resource definitions and instana import blocks:

```hcl

# Import blocks (generated by script)
import {
  to = instana_log_alert_config.my_log_alert
  id = "YzrwxoGDS0mY77DBk_8EqQ"
}
```

#### Step 8: Initialize with New Provider
...
# Optional: Move back to your original directory
cd ..

# Keep the old state backup for rollback purposes
...
- The **old state file** (backup) should be kept for rollback purposes
- Do NOT copy the old state file to the migration directory - let Terraform create a new one
- After successful migration, you can use the new state file going forward
...
- [ ] Copy import blocks from `import.tf` to `main.tf`
...
- [ ] Use new state file (`migration/terraform.tfstate`) going forward
