...
func TestNewAPITokenResourceHandleFramework(t *testing.T) {
	handle := NewAPITokenResourceHandleFramework()
	require.NotNil(t, handle)

	metadata := handle.MetaData()
	require.NotNil(t, metadata)
	assert.Equal(t, ResourceInstanaAPITokenFramework, metadata.ResourceName)
	assert.Equal(t, int64(2), metadata.SchemaVersion)
	assert.NotNil(t, metadata.Schema)
}

func TestMetaData(t *testing.T) {
	resource := &apiTokenResourceFramework{
