...
func TestNewCustomDashboardResourceHandleFramework(t *testing.T) {
	handle := NewCustomDashboardResourceHandleFramework()
	require.NotNil(t, handle)

	metadata := handle.MetaData()
	require.NotNil(t, metadata)
	assert.Equal(t, ResourceInstanaCustomDashboardFramework, metadata.ResourceName)
