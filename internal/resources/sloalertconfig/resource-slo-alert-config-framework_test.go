...
func TestUpdateState_StatusAlertType(t *testing.T) {
	...
	resource := NewSloAlertConfigResourceHandleFramework()
...
func TestMetaData(t *testing.T) {
	...
	assert.Equal(t, ResourceInstanaSloAlertConfigFramework, metaData.ResourceName)
	assert.Equal(t, int64(1), metaData.SchemaVersion)
	assert.False(t, metaData.CreateOnly)
}

func TestNewSloAlertConfigResourceHandleFramework(t *testing.T) {
	resource := NewSloAlertConfigResourceHandleFramework()
