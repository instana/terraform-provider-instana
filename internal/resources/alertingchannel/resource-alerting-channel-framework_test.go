...
func TestNewAlertingChannelResourceHandleFramework(t *testing.T) {
	handle := NewAlertingChannelResourceHandleFramework()
	require.NotNil(t, handle)

	metadata := handle.MetaData()
	require.NotNil(t, metadata)
	assert.Equal(t, ResourceInstanaAlertingChannelFramework, metadata.ResourceName)
	assert.Equal(t, int64(1), metadata.SchemaVersion)
	assert.NotNil(t, metadata.Schema)
}

func TestMetaData(t *testing.T) {
	resource := &alertingChannelResourceFramework{
