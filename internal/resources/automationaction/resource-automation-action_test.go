...
func TestMapStateToDataObject_ScriptAction(t *testing.T) {
	...
	contentField := findField(result.Fields, instana.ScriptSshFieldName)
...
func TestMapStateToDataObject_HttpAction(t *testing.T) {
	...
	// Verify HTTP fields
	hostField := findField(result.Fields, instana.HttpHostFieldName)
	require.NotNil(t, hostField)
	assert.Equal(t, "https://example.com/api", hostField.Value)

	methodField := findField(result.Fields, instana.HttpMethodFieldName)
...
func TestMapStateToDataObject_HttpActionWithHeaders(t *testing.T) {
	...
	headerField := findField(result.Fields, instana.HttpHeaderFieldName)
...
func TestUpdateState_EmptyInputParameters(t *testing.T) {
	...
	data := &instana.AutomationAction{
		...
		Type:            AutomationActionTypeManual,
		Tags:            nil,
		Fields:          []instana.Field{{Name: AutomationActionFieldContent, Value: "Manual content"}},
		InputParameters: []instana.Parameter{},
	}
...
func TestMapActionTypeAndFields_ScriptWithAllFields(t *testing.T) {
	...
	interpreterField := findField(result.Fields, instana.SubtypeFieldName)
	require.NotNil(t, interpreterField)
	assert.Equal(t, "bash", interpreterField.Value)

	timeoutField := findField(result.Fields, instana.TimeoutFieldName)
...
func TestMapActionTypeAndFields_HttpWithAllOptionalFields(t *testing.T) {
	...
	bodyField := findField(result.Fields, instana.HttpBodyFieldName)
	require.NotNil(t, bodyField)
	assert.Equal(t, `{"test":"data"}`, bodyField.Value)

	ignoreCertField := findField(result.Fields, instana.HttpIgnoreCertErrorsFieldName)
...
func TestUpdateState_AllActionTypes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			fields:     []instana.Field{{Name: AutomationActionFieldProject, Value: "PROJ"}},
		},
		{
			name:       "GitHub",
			actionType: AutomationActionTypeGitHub,
			fields:     []instana.Field{{Name: AutomationActionFieldOwner, Value: "owner"}},
		},
		{
			name:       "DocLink",
			actionType: AutomationActionTypeDocLink,
			fields:     []instana.Field{{Name: AutomationActionFieldUrl, Value: "https://docs.example.com"}},
		},
		{
			name:       "GitLab",
			actionType: AutomationActionTypeGitLab,
			fields:     []instana.Field{{Name: AutomationActionAPIFieldProjectId, Value: "123"}},
		},
		{
			name:       "Ansible",
			actionType: AutomationActionTypeAnsible,
			fields:     []instana.Field{{Name: AutomationActionAPIFieldWorkflowId, Value: "workflow-123"}},
		},
	}
...
func findField(fields []instana.Field, name string) *instana.Field {
...
func TestMapActionTypeAndFields_HTTPWithHeaders(t *testing.T) {
	...
	// Find headers field
	var headersField *instana.Field
	for i := range fields {
		if fields[i].Name == instana.HttpHeaderFieldName {
