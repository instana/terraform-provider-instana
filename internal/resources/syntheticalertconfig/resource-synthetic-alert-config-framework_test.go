...
func TestNewSyntheticAlertConfigResourceHandleFramework(t *testing.T) {
	t.Run("should create resource handle with correct metadata", func(t *testing.T) {
		handle := NewSyntheticAlertConfigResourceHandleFramework()

		require.NotNil(t, handle)
		metadata := handle.MetaData()
		require.NotNil(t, metadata)
		assert.Equal(t, ResourceInstanaSyntheticAlertConfigFramework, metadata.ResourceName)
...
func TestSetComputedFields(t *testing.T) {
	t.Run("should return nil diagnostics", func(t *testing.T) {
		resource := &syntheticAlertConfigResourceFramework{
			metaData: resourcehandle.ResourceMetaDataFramework{
				ResourceName:  ResourceInstanaSyntheticAlertConfigFramework,
				Schema:        NewSyntheticAlertConfigResourceHandleFramework().MetaData().Schema,
				SchemaVersion: 1,
			},
		}
