...
type SyntheticTestModel struct {
	...
	CustomProperties map[string]string          `tfsdk:"custom_properties"`
	Locations        []string                   `tfsdk:"locations"`
...
type HttpActionConfigModel struct {
	...
	ExpectExists      []string          `tfsdk:"expect_exists"`
	ExpectNotEmpty    []string          `tfsdk:"expect_not_empty"`
	ExpectJson        map[string]string `tfsdk:"expect_json"`
}
...
type DNSConfigModel struct {
	...
	TargetValues      []DNSFilterTargetValueModel `tfsdk:"target_values"`
}
...
type SSLCertificateConfigModel struct {
	...
	Port                 types.Int64                     `tfsdk:"port"`
	ValidationRules      []SSLCertificateValidationModel `tfsdk:"validation_rules"`
}
