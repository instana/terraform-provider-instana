...
func TestNewApplicationAlertConfigResourceHandleFramework(t *testing.T) {
	resource := NewApplicationAlertConfigResourceHandleFramework()
	require.NotNil(t, resource)

	metaData := resource.MetaData()
	assert.Equal(t, ResourceInstanaApplicationAlertConfigFramework, metaData.ResourceName)
	assert.NotNil(t, metaData.Schema)
	assert.Equal(t, int64(1), metaData.SchemaVersion)
	assert.True(t, metaData.SkipIDGeneration)
}

func TestNewGlobalApplicationAlertConfigResourceHandleFramework(t *testing.T) {
	resource := NewGlobalApplicationAlertConfigResourceHandleFramework()
	require.NotNil(t, resource)

	metaData := resource.MetaData()
	assert.Equal(t, ResourceInstanaGlobalApplicationAlertConfigFramework, metaData.ResourceName)
	assert.NotNil(t, metaData.Schema)
	assert.Equal(t, int64(1), metaData.SchemaVersion)
}

func TestMetaData(t *testing.T) {
	resource := &applicationAlertConfigResourceFramework{
...
func TestResourceFrameworkImpl_GetID(t *testing.T) {
...
func TestResourceFrameworkImpl_SetID(t *testing.T) {
	resource := &applicationAlertConfigResourceFrameworkImpl{}
	data := &restapi.ApplicationAlertConfig{}
...
func TestMapStateToDataObject_WithRules(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: ApplicationAlertConfigFieldRuleErrorRate,
		},
		{
			name: "errors",
			rule: &RuleModel{
			...
			expected: ApplicationAlertConfigFieldRuleErrors,
		},
		{
			name: "logs",
			rule: &RuleModel{
			...
			expected: ApplicationAlertConfigFieldRuleLogs,
		},
		{
			name: "slowness",
			rule: &RuleModel{
			...
			expected: ApplicationAlertConfigFieldRuleSlowness,
		},
		{
			name: "status_code",
			rule: &RuleModel{
				StatusCode: &StatusCodeRuleModel{
					...
					Aggregation:     types.StringValue("sum"),
					StatusCodeStart: types.Int64Value(500),
					StatusCodeEnd:   types.Int64Value(599),
				},
			},
			expected: ApplicationAlertConfigFieldRuleStatusCode,
		},
		{
			name: "throughput",
			rule: &RuleModel{
			...
			expected: ApplicationAlertConfigFieldRuleThroughput,
		},
	}
...
func TestUpdateState_WithRulesAndThresholds(t *testing.T) {
	...
	assert.Equal(t, int64(100), model.Rules[0].Thresholds.Warning.Static.Value.ValueInt64())
}
...
func TestNewResource(t *testing.T) {
	...
	assert.IsType(t, &applicationAlertConfigResourceFrameworkImpl{}, impl)
}
...
func TestNewResource_Global(t *testing.T) {
	...
	implTyped := impl.(*applicationAlertConfigResourceFrameworkImpl)
...
func TestMapStateToDataObject_WithNullSeverity(t *testing.T) {
	...
	assert.Equal(t, 0, int(result.Severity))
}
...
func TestMapStateToDataObject_WithNullAdaptiveBaselineFields(t *testing.T) {
	...
	threshold := result.Rules[0].Thresholds[restapi.WarningSeverity]
	assert.Equal(t, "adaptiveBaseline", threshold.Type)
	// Null fields should result in nil pointers
	assert.Nil(t, threshold.DeviationFactor)
	assert.Nil(t, threshold.Adaptability)
	assert.Nil(t, threshold.Seasonality)
}
