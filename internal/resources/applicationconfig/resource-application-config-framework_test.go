...
func TestNewApplicationConfigResourceHandleFramework(t *testing.T) {
	resource := NewApplicationConfigResourceHandleFramework()
	require.NotNil(t, resource)

	metaData := resource.MetaData()
	assert.Equal(t, ResourceInstanaApplicationConfigFramework, metaData.ResourceName)
	assert.NotNil(t, metaData.Schema)
	assert.Equal(t, int64(4), metaData.SchemaVersion)
}

func TestMetaData(t *testing.T) {
	resource := &applicationConfigResourceFramework{
