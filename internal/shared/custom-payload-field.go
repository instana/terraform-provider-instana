...
func CustomPayloadFieldsToTerraform(ctx context.Context, fields []instana.CustomPayloadField[any]) (types.List, diag.Diagnostics) {
	...
	for _, field := range fields {
		...
		if field.Type == instana.StaticStringCustomPayloadType {
			// Static string value
			staticValue, ok := field.Value.(string)
			if !ok {
				// Try to convert from the custom type
				if customValue, ok := field.Value.(instana.StaticStringCustomPayloadFieldValue); ok {
		...
		} else if field.Type == instana.DynamicCustomPayloadType {
			// Dynamic value
			dynamicValue, ok := field.Value.(instana.DynamicCustomPayloadFieldValue)
			if !ok {
				diags.AddError(
					"Error converting custom payload field",
...
func MapCustomPayloadFieldsToAPIObject(ctx context.Context, customPayloadFieldsList types.List) ([]instana.CustomPayloadField[any], diag.Diagnostics) {
	var diags diag.Diagnostics

	// handle null/unknown
	if customPayloadFieldsList.IsNull() || customPayloadFieldsList.IsUnknown() {
		return []instana.CustomPayloadField[any]{}, diags
	}
	...
	out := make([]instana.CustomPayloadField[any], 0, len(elems))
	...
	for idx, e := range elems {
		...
		if !e.DynamicValue.IsNull() && !e.DynamicValue.IsUnknown() {
			...
			dynValue := instana.DynamicCustomPayloadFieldValue{
				Key:     keyPtr,
				TagName: tagName,
			}
			...
			out = append(out, instana.CustomPayloadField[any]{
...
func MapStaticOnlyCustomPayloadFieldsToAPIObject(ctx context.Context, customPayloadFieldsList types.List) ([]instana.CustomPayloadField[any], diag.Diagnostics) {
	var diags diag.Diagnostics
...
func StaticOnlyCustomPayloadFieldsToTerraform(ctx context.Context, fields []instana.CustomPayloadField[any]) (types.List, diag.Diagnostics) {
	var diags diag.Diagnostics
