...
1. **Get the Resource ID**: First, locate the ID of your synthetic test in Instana. You can find this in the Instana UI or via the API.

2. **Create an Import Block**: Create a new `.tf` file (e.g., `import.tf`) with an import block:

```hcl
import {
  to = instana_synthetic_test.example
  id = "resource_id"
}
```
...
- `resource_id` with your actual synthetic test ID from Instana
