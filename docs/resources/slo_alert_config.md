...
1. **Get the Resource ID**: First, locate the ID of your SLO alert configuration in Instana. You can find this in the Instana UI or via the API.

2. **Create an Import Block**: Create a new `.tf` file (e.g., `import.tf`) with an import block:

```hcl
import {
  to = instana_slo_alert_config.example
  id = "resource_id"
}
```
...
- `resource_id` with your actual SLO alert configuration ID from Instana
